 ...
 *
 * 최초 선정 공시일 다음날부터 휘발유 또는 경유가 단위지역의 해당 유종 평균을
 * 초과한 날이 이 값 이상이면 검토 대상으로 본다. 두 유종 동시 초과는 하루로 센다.
 * 2026-09-09 사용자 제공 기준. 실제 취소 처분을 의미하지 않는다.
 */
...
export interface OverRegion {
  ...
  /** 그중 지역 평균을 넘긴 날 수 */
  overDays: number;
  /** 초과일마다 두 유종 중 큰 초과액을 택한 값의 평균 (원) */
  meanOver: number;
  /** 개별 유종의 최대 초과액과 그 날짜 */
  maxOver: number;
  maxDate: string;
