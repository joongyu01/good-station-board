...
| 코드 | 상호 | 사유 |
|---|---|---|
| `A0026263` | 대황주유소 (충북 제천) | 오피넷 전국 목록에 없음 — 폐업 또는 가격 미신고 |
| `A0000686` | (주)디오티디 은평유니콘주유소 | 그날 두 유종 모두 미신고 |
| `A0023664` | 대광셀프주유소 (경북 칠곡) | 경유 미신고 → 합산·경유 기준만 미상 |
...
- **상세 단계**에서만 실제 지도 타일 + **주유기 아이콘 핀**. 전국에 472개를 찍으면
  알아볼 수 없습니다.
- **상호나 아이콘을 누르면** 판매가 추이 창 (좌축 가격, 우축 계수·빨간 선,
  1.000 커트라인 점선).
...
추이 차트의 계수 선은 **통합 기준 하나뿐**입니다. 기준을 휘발유로 바꿔도 차트의
계수는 통합값입니다. 유종별 계수 추이가 필요하면 66일치를 다시 계산해야 합니다
...
자동 실행은 매일 **10:20 KST**. 코드만 바꿨을 때는 `skip_collect` 를 쓰세요 —
NetFunnel 대기열이 길면 20분 넘게 잡힙니다.
