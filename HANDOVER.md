...
**보정 판정의 정확도를 가장 크게 좌우하는 항목입니다.**

차수가 곧 선정 기준기간인데, 보관 원본이 7/1부터라 그 이전을 볼 수 없습니다.
데이터로 확인된 것은 둘뿐입니다.

| 차수 | 기준기간 | 근거 |
|---|---|---|
| 8차 | 2026-07 | 7월 기준 top-N 충족률 80%, 순위 중앙값 3위 |
| 9차 | 2026-08 | 8월 기준 59% (7월 기준으로는 5%) |
| 1~7차 | **모름 — 7월로 갈음** | 관측 시작 시점이라 그 이전을 못 본다 |

지금 379곳이 7월 기준으로 재고 있는데, 그 차수들은 **'관측을 시작한 7월 대비'**
를 재는 셈이라 7월 이전에 올린 몫이 잡히지 않습니다. **실제보다 후하게 나옵니다.**
화면(관리 화면 비교표)에도 그렇게 밝혀 두었습니다.

**할 일** — 차수별 선정 기준기간(몇 년 몇 월분 가격으로 뽑았는지)을 확인해서
`data/round-windows.json` 에 적으세요. 형식은 `{"1차":"202603", …}` 입니다.
그 다음 그 달 오피넷 원본을 백필하면(`npm run backfill -- --redownload`) 제대로
잽니다. 명단 원본에 기간을 적는 열이 하나 있긴 한데 **전부 비어 있어** 거기서는
얻지 못했습니다.
