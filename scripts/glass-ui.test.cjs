...
(async () => {
  ...
  await stats.getByRole('button',{name:'시계열 그래프',exact:true}).click();
  const trend=stats.locator('.region-trend-modal');
  assert.equal(await trend.getByRole('tab',{name:'절감률',exact:true}).getAttribute('aria-selected'),'true');
  assert.equal(await trend.locator('.savings-legend button').count(),18);
  await trend.screenshot({path:'.tmp/preview/regional-savings.png'});
  const priceStats=JSON.parse(fs.readFileSync('client/public/data/region-prices.json','utf8'));
  const last=Object.keys(priceStats.days).sort().at(-1);
  await trend.getByLabel('절감률 시작일').selectOption(last);
  assert.equal(await trend.locator('.savings-legend button').count(),16);
  assert.equal(await trend.getByRole('slider').getAttribute('aria-valuemax'),'1');
  await trend.getByRole('tab',{name:'분포',exact:true}).click();
  const sd=priceStats.days[last]['서울'].gasoline.sd.toLocaleString('ko-KR',{minimumFractionDigits:2,maximumFractionDigits:2});
  assert.ok((await trend.locator('.savings-legend button').filter({hasText:'서울'}).innerText()).includes(sd+'원/L'));
  await trend.getByLabel('분포 지표').selectOption('median');
  await trend.getByLabel('절감률 유종').selectOption('diesel');
  await trend.getByLabel('절감률 시작일').selectOption('20260401');
  await trend.screenshot({path:'.tmp/preview/regional-distribution.png'});
  await trend.getByRole('button',{name:'시계열 그래프 닫기'}).click();
  await stats.getByRole('button',{name:'시계열 그래프',exact:true}).click();
  assert.equal(await trend.getByRole('tab',{name:'절감률',exact:true}).getAttribute('aria-selected'),'true');
