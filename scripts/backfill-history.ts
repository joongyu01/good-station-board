 ...
 *
 * 받은 원본은 gzip 으로 남긴다(data/raw). 판정식이 바뀌어 두 달치를 다시
 * 낼 때 오피넷을 다시 긁지 않고 보관분으로 재계산한다 — 세 시간이 몇 초가 된다.
 *
 * 원본이 없는 날짜만 내려받는다. 중간에 끊겨도 다시 실행하면 이어서 채운다.
 *
 * 실행:
 ...
 *
 * 옵션 앞의 `--` 를 빼면 npm 이 먹어치워 스크립트까지 오지 않는다. 날짜 인자는
 * `--` 없이도 넘어간다.
 */
